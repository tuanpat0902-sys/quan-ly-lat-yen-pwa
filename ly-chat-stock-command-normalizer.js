...
function explicitOperation(source){if(/(^|\s)(nhap|nhap kho|nhap hang)(\s|$)/.test(source))return 'Nhập';if(/(^|\s)(xuat|xuat kho|xuat hang)(\s|$)/.test(source))return 'Xuất';if(/kiem ke|kiem kho|phieu kiem/.test(source))return 'Kiểm kê';return '';}
...
function resolveIngredient(term){
  const rows=ingredients(),needle=norm(term);if(!needle)return {mode:'none'};
  const exact=rows.find(x=>norm(x.name)===needle);if(exact)return {mode:'exact',item:exact};
  const tokens=needle.split(' ').filter(Boolean);
  if(tokens.length===1){
    const firstWord=rows.filter(item=>norm(item.name).split(' ')[0]===needle);
    if(firstWord.length===1)return {mode:'alias',item:firstWord[0]};
    if(firstWord.length>1)return {mode:'missing',partial:firstWord};
  }
...
function process(input){
  const source=norm(input.value),op=contextualOperation(source,input.value);if(!op)return {allow:true};
  const amount=amountOf(input.value),term=requestedTerm(input.value),resolved=resolveIngredient(term);
  if(resolved.mode==='exact'||resolved.mode==='alias'||resolved.mode==='typo'){
...
function install(){const drawer=document.getElementById?.('lyAssistantDrawer');if(!drawer||drawer.dataset.stockCommandNormalizer==='2')return false;drawer.dataset.stockCommandNormalizer='2';const handler=e=>{const send=e.target?.closest?.('[data-assistant-send]'),enter=e.type==='keydown'&&e.key==='Enter'&&!e.shiftKey;if(!send&&!enter)return;const input=document.getElementById?.('lyAssistantInput');if(!input?.value)return;const r=process(input);if(r.allow)return;e.preventDefault?.();e.stopImmediatePropagation?.();};drawer.addEventListener('click',handler,true);drawer.querySelector?.('#lyAssistantInput')?.addEventListener('keydown',handler,true);return true;}
function boot(i=0){if(install())return;if(i<50)setTimeout(()=>boot(i+1),100);}
window.__lyChatStockCommandNormalizer={version:VERSION,requestedTerm,resolveIngredient,convert,process,install,status:()=>({version:VERSION,installed:document.getElementById?.('lyAssistantDrawer')?.dataset?.stockCommandNormalizer==='2',context:{...context}})};
