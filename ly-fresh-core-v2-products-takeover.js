(()=>{
  ...
  function coreReady(){
    ...
    return core?.domains?.products?.save?core:null;
  }
  ...
  async function routedRpc(name,params,...rest){
    ...
    try{
      ...
      state.errors++;
      state.lastError=String(error?.message||error||'V2 product save failed');
  ...
  function enable(){
    ...
    Object.defineProperty(wrapper,'__lyV2ProductsTakeover',{value:true});
    client.rpc=wrapper;
    state.enabled=true;
    state.phase='active';
    state.lastError='';
