(()=>{
  ...
  function wordSpans(raw){
    const spans=[];
    for(const match of String(raw).matchAll(/[\p{L}\p{M}0-9]+/gu))spans.push({raw:match[0],folded:fold(match[0]),start:match.index,end:Number(match.index)+match[0].length});
    return spans;
  }
  function replacePhrase(raw,target,replacement){
    const targetWords=fold(target).split(' ').filter(Boolean);if(!targetWords.length)return raw;
    let result=String(raw),searchFrom=0;
    while(true){
      const spans=wordSpans(result).filter(row=>row.end>searchFrom);let found=null;
      for(let i=0;i<=spans.length-targetWords.length;i++){
        const slice=spans.slice(i,i+targetWords.length);
        if(slice.every((row,index)=>row.folded===targetWords[index])){found=slice;break;}
      }
      if(!found)break;
      const start=found[0].start,end=found.at(-1).end;
  ...
  function patchAssistant(){
    ...
    for(const key of ['assistantReply','reportReply','contextualReportMessage']){
      const original=typeof assistant[key]==='function'?assistant[key].bind(assistant):null;if(!original)continue;
      assistant[key]=(message,...rest)=>original(normalizeMessage(message),...rest);
    }
