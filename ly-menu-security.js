(()=>{
  'use strict';
  if(window.__lyMenuSecurityV12)return;
  window.__lyMenuSecurityV12=true;

  const VERSION='2026.08.23.1.2';
  ...
  const state={enabled:cachedEnabled,loaded:false,unlockUntil:0,pending:null,timer:null,observer:null,baseShowTab:null,guard:null,renderScheduled:false};
  ...
  function protectedButtonForPanel(panel){
    const direct=document.querySelector(`#nav button[data-panel="${CSS.escape(text(panel))}"]`);
  ...
  function markMenus(){
    document.querySelectorAll('#nav button[data-panel]').forEach(btn=>{
      ...
      if(!protectedMenu){btn.classList.remove('ly-protected-menu','ly-protected-unlocked');btn.querySelector('.ly-menu-lock-mark')?.remove();return;}
      ...
      if(!state.enabled){mark?.remove();return;}
      if(!mark){mark=document.createElement('span');mark.className='ly-menu-lock-mark';btn.appendChild(mark);}
      mark.innerHTML=lockSvg(isUnlocked());mark.title=isUnlocked()?'Đã mở khóa tạm thời':'Cần mật khẩu';
    });
  }
  ...
  function ensureModal(){
    ...
    ov.addEventListener('click',e=>{if(e.target===ov)closeModal();});ov.querySelector('#lyMenuSecurityCancel').onclick=closeModal;ov.querySelector('#lyMenuSecuritySubmit').onclick=verifyFromModal;ov.querySelector('#lyMenuSecurityPassword').addEventListener('keydown',e=>{if(e.key==='Enter')verifyFromModal();});document.body.appendChild(ov);return ov;
  }
  ...
  async function verifyFromModal(){
    ...
    try{const {data,error}=await client.rpc('ly_verify_menu_password',{p_password:password});if(error)throw error;if(data!==true){if(err)err.textContent='Mật khẩu không đúng.';input.select();return;}state.unlockUntil=Date.now()+UNLOCK_MS;const pending=state.pending;document.getElementById('lyMenuSecurityOverlay')?.classList.remove('open');state.pending=null;markMenus();renderSettingsCard();notify('Đã mở khóa các menu được bảo vệ trong 10 phút.');if(pending)state.baseShowTab?.call(window,pending.panel,pending.btn);}catch(e){if(err)err.textContent='Không thể xác minh lúc này. Vui lòng thử lại.';console.warn('[Lát Yên] menu password verify',e);}finally{if(submit){submit.disabled=false;submit.textContent='Mở khóa';}}
  }

  async function loadStatus(force=false){
    if(state.loaded&&!force)return state.enabled;const client=getClient();if(!client)return state.enabled;
    try{const {data,error}=await client.rpc('ly_menu_password_status');if(error)throw error;const row=Array.isArray(data)?data[0]:data;state.enabled=typeof row==='boolean'?row:!!row?.enabled;state.loaded=true;try{localStorage.setItem(ENABLE_CACHE,state.enabled?'1':'0');}catch(e){}if(!state.enabled)state.unlockUntil=0;markMenus();renderSettingsCard();return state.enabled;}catch(e){console.warn('[Lát Yên] menu password status',e);return state.enabled;}
  }
  ...
  function ensureSettingsCard(){
    ...
    let card=document.getElementById('lyMenuSecuritySettings');
    if(card&&!panel.contains(card)){card.remove();card=null;}
    if(!card){card=document.createElement('div');card.id='lyMenuSecuritySettings';card.className='card ly-security-card';panel.appendChild(card);}
    return card;
  }
  function renderSettingsCard(){
    ...
    card.querySelector('#lySecSave')?.addEventListener('click',savePassword);card.querySelector('#lySecLockNow')?.addEventListener('click',()=>{state.unlockUntil=0;markMenus();renderSettingsCard();notify('Đã khóa lại các menu được bảo vệ.');});card.querySelector('#lySecDisable')?.addEventListener('click',disablePassword);
  }
  ...
  function scheduleSettingsRender(){if(state.renderScheduled)return;state.renderScheduled=true;setTimeout(()=>{state.renderScheduled=false;ensureSettingsRendered();},80);}
  ...
  async function savePassword(){
    ...
    const next=nw.value,confirm=cf.value,current=cur?.value||'';if(next.length<4){notify('Mật khẩu cần ít nhất 4 ký tự.',false);nw.focus();return;}if(next!==confirm){notify('Xác nhận mật khẩu chưa khớp.',false);cf.focus();return;}if(state.enabled&&!current){notify('Vui lòng nhập mật khẩu hiện tại.',false);cur?.focus();return;}
    if(btn){btn.disabled=true;btn.textContent='Đang lưu…';}
    try{const {data,error}=await client.rpc('ly_set_menu_password',{p_new_password:next,p_current_password:current||null});if(error)throw error;if(data!==true){notify('Mật khẩu hiện tại không đúng.',false);return;}state.enabled=true;state.loaded=true;try{localStorage.setItem(ENABLE_CACHE,'1');}catch(e){}state.unlockUntil=0;markMenus();renderSettingsCard();notify('Đã cập nhật mật khẩu bảo vệ menu.');}catch(e){console.warn('[Lát Yên] set menu password',e);notify('Không thể lưu mật khẩu. Kiểm tra quyền quản trị và thử lại.',false);}finally{if(btn)btn.disabled=false;}
  }
  async function disablePassword(){
    ...
    try{const {data,error}=await client.rpc('ly_disable_menu_password',{p_current_password:current});if(error)throw error;if(data!==true){notify('Mật khẩu hiện tại không đúng.',false);return;}state.enabled=false;state.loaded=true;try{localStorage.setItem(ENABLE_CACHE,'0');}catch(e){}state.unlockUntil=0;markMenus();renderSettingsCard();notify('Đã tắt bảo vệ menu.');}catch(e){console.warn('[Lát Yên] disable menu password',e);notify('Không thể tắt bảo vệ lúc này.',false);}
  }

  function installGuard(){
    if(typeof window.showTab!=='function')return;
    if(window.showTab===state.guard)return;
    ...
    state.guard=function(id,btn){
      ...
      if(!state.enabled)return state.baseShowTab.apply(this,arguments);
      openModal(panel,button);return false;
    };
  ...
  function start(){
    ...
    state.observer?.disconnect();state.observer=new MutationObserver(()=>{scheduleSettingsRender();markMenus();installGuard();});state.observer.observe(document.body,{subtree:true,childList:true});
    clearInterval(state.timer);state.timer=setInterval(()=>{installGuard();markMenus();ensureSettingsRendered();if(state.enabled&&state.unlockUntil&&Date.now()>=state.unlockUntil){state.unlockUntil=0;markMenus();renderSettingsCard();}},900);
  }
