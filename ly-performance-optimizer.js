(()=>{
  ...
  function nextDelay(){if(!navigator.onLine)return OFFLINE_MS;if(VIBE_ONLY)return document.hidden?HIDDEN_MS:VIBE_REFRESH_MS;if(pendingCount()>0)return PENDING_MS;if(document.hidden)return HIDDEN_MS;if(quietHours())return QUIET_MS;return realtimeLive()?LIVE_MS:FALLBACK_MS;}
  ...
  function start(){installRenderStyles();clearLegacy();try{v268StartCloudHeartbeat=()=>{clearLegacySyncTimers();schedule(VIBE_ONLY?VIBE_REFRESH_MS:1200);};}catch(e){}try{v269StartSyncEngine=()=>{clearLegacySyncTimers();schedule(VIBE_ONLY?VIBE_REFRESH_MS:1200);};}catch(e){}installScopedTableObserver();applyVisibilityState();schedule(VIBE_ONLY?VIBE_REFRESH_MS:1800);scheduleLeader(700);}

  document.addEventListener('visibilitychange',applyVisibilityState);window.addEventListener('storage',onLeaderStorage);window.addEventListener('online',()=>schedule(VIBE_ONLY?VIBE_REFRESH_MS:300));window.addEventListener('offline',()=>schedule(OFFLINE_MS));window.addEventListener('beforeunload',()=>{clearTimeout(state.timer);clearTimeout(state.leaderTimer);clearTimeout(state.trimTimer);state.tableObserver?.disconnect();clearLegacyLeaderTimer();},{once:true});
