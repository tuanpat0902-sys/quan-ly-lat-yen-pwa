-- Optional per-warehouse password protection. Password hashes stay in the
-- non-exposed ly_private schema and are never returned to the browser.
create table if not exists ly_private.ly_warehouse_passwords (
  warehouse_id uuid primary key references public.ly_warehouses(id) on delete cascade,
...
alter table ly_private.ly_warehouse_passwords enable row level security;

drop policy if exists ly_warehouse_password_admin_all on ly_private.ly_warehouse_passwords;
create policy ly_warehouse_password_admin_all
on ly_private.ly_warehouse_passwords
for all
to authenticated
using (
...
create or replace function public.ly_save_warehouse_secure(
  p_warehouse jsonb,
  p_password_mode text default 'keep',
...
begin
  ...
  -- Receipt and sale child rows cascade from their headers. Remaining
  -- warehouse-scoped rows cascade when the warehouse itself is removed.
...
grant execute on function public.ly_warehouse_password_status(uuid) to authenticated;
grant execute on function public.ly_save_warehouse_secure(jsonb, text, text, text) to authenticated;
