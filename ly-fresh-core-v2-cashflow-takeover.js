(()=>{
  ...
  const state={version:VERSION,phase:'waiting',enabled:false,calls:0,success:0,errors:0,fallbacks:0,lastAction:'',lastAt:0,lastError:''};
  let client=null;
  ...
  function coreReady(){
    ...
    const cashflow=core?.domains?.cashflow;
    return cashflow?.refresh&&cashflow?.create&&cashflow?.update&&cashflow?.remove?core:null;
  }

  function response(data,error=null){
    return error
      ?{data:null,error,status:400,statusText:'V2 cashflow mutation failed'}
  ...
  async function saveEntry(entry){
    ...
    if(!core)throw new Error('Fresh Core V2 cashflow is not ready');
    ...
    const data=exists
      ?await core.domains.cashflow.update(entry.id,entry)
      :await core.domains.cashflow.create(entry);
    state.success++;
    state.lastError='';
  ...
  function deleteBuilder(){
    ...
    let executed=null;
    const builder={
      eq(column,value){filters[String(column)]=value;return builder;},
    ...
    async function execute(){
      ...
      executed=(async()=>{
        ...
        if(!core)throw new Error('Fresh Core V2 cashflow is not ready');
        const id=filters.id;
        if(!id)throw new Error('Cashflow delete requires id');
        ...
        state.errors++;
        state.lastError=String(error?.message||error||'V2 cashflow mutation failed');
  ...
  function cashflowTable(rawTable){
    return new Proxy(rawTable,{
      get(target,prop,receiver){
        if(prop==='upsert')return async(entry)=>{
          try{return await saveEntry(entry);}catch(error){state.errors++;state.lastError=String(error?.message||error);return response(null,error);}
        };
  ...
  function enable(){
    ...
    if(!client||typeof client.from!=='function'||!coreReady())return false;
    if(client.from?.__lyV2CashflowTakeover)return true;
    previousFrom=client.from.bind(client);
    const wrapper=function(name,...rest){
      const raw=previousFrom(name,...rest);
      return String(name)===TABLE?cashflowTable(raw):raw;
    };
