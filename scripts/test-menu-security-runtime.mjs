...
assert.match(security,/VERSION='2026\.08\.24\.2'/);
...
assert.ok(loader.includes("menuSecurity:{src:'./ly-menu-security.js?v=20260824.2'"),'module loader must fetch menu security');
assert.ok(loader.includes("await load('menuSecurity')"),'module loader must activate menu security');
assert.ok(pages.includes('<script src="./ly-menu-security.js?v=20260824.2"></script>'),'Pages fallback runtime must include menu security');
