...
const [tableView,legacyTables,app,index,employees,history,sw,runtime]=await Promise.all([
...
assert.match(tableView,/VERSION='2026\.08\.28\.1'/,'Table View V2 version missing');
for(const key of ['suppliers','employees','activity'])assert.match(tableView,new RegExp(`${key}:Object\\.freeze`),`${key} registry contract missing`);
...
assert.match(app,/ly-table-view-v2\.js\?v=20260828\.1/,'V2 asset must be cache-busted');
...
assert.equal(release.uiBuild,'UI-2026.08.28.12');
assert.equal(release.serviceWorker,'lat-yen-fresh-core-v3-authoritative-204');
assert.match(release.tableViewV2,/pilot-suppliers-employees-activity/);
