...
for(const fragment of ['ly_runtime_migrations','pg_advisory_xact_lock','ly_idx_sales_org_wh_sold','ly_idx_stock_tx_org_wh_created','ly_idx_activity_org_id','create sequence if not exists','pg_indexes','actor_email'])assert.ok(source.includes(fragment),`missing schema safeguard: ${fragment}`);
