...
for(const fragment of ['ly_runtime_migrations','pg_advisory_xact_lock','ly_idx_sales_org_wh_sold','ly_idx_stock_tx_org_wh_created','ly_idx_activity_org_id','create sequence if not exists','pg_indexes','actor_email','add column if not exists updated_at','entered_quantity','entered_unit','conversion_ratio','inventoryRepairs','rebuildDocumentLedger','rebuildVibeIposInventory','repairImpossiblePositiveInventory','ly_inventory_reconciliation_audit'])assert.ok(source.includes(fragment),`missing schema safeguard: ${fragment}`);
assert.match(source,/delete from .*ly_stock_transactions.*transaction_type in\('IMPORT','EXPORT','ADJUSTMENT'\)/s,'document ledger must be rebuilt only from authoritative receipt rows');
assert.match(source,/for\(const org of orgs\)inventoryRepairs\+=\(await repairImpossiblePositiveInventory/,'every startup must reconcile cached balances with the complete ledger');
