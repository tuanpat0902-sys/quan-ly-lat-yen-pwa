...
context.window.__lyUnitConversions={canonical:value=>({goi:'gói'}[String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase()]||String(value||'').toLowerCase()),convert:(quantity,from,to,id)=>id==='i8'&&from==='gói'&&to==='g'?Number(quantity)*500:Number.NaN};
...
const coffeeBeans=assistant.parseDraft('Nhập 10 gói hạt cà phê');assert.equal(coffeeBeans.clarifications.length,1);assert.equal(coffeeBeans.clarifications[0].query.toLocaleLowerCase('vi'),'hạt cà phê');assert.equal(coffeeBeans.clarifications[0].options.map(row=>row.name).join('|'),'Hạt cà phê 37','a multi-word ingredient must never degrade to the one-word “cà” or suggest a milk-frothing jug');
assert.equal(assistant.chooseDraftItem(coffeeBeans,coffeeBeans.clarifications[0].id,'i8'),true);assert.equal(coffeeBeans.items[0].quantity,5000,'10 purchase packs of 500 g must become 5,000 g in inventory');
...
const ambiguousConvertedPrice=assistant.parseDraft('Nhập 10 kg bột đơn giá 10 nghìn');assert.equal(assistant.chooseDraftItem(ambiguousConvertedPrice,ambiguousConvertedPrice.clarifications[0].id,'i6'),true);assert.equal(ambiguousConvertedPrice.items[0].quantity,10000);assert.equal(ambiguousConvertedPrice.items[0].unit_cost,10,'suggestion selection must convert both quantity and quoted unit price to the selected base unit');
