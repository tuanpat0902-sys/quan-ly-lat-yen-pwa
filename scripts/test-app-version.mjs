...
assert.ok(loader.includes('ly-warehouse-delete-ux.js?v=20260824.1'),'module loader must bootstrap warehouse delete UX');
assert.ok(warehouseDeleteUx.includes("code==='23503'"),'warehouse UX must sanitize foreign-key deletion failures');
assert.ok(warehouseDeleteUx.includes('Không thể xóa'),'warehouse UX must explain blocked deletion in Vietnamese');
