...
assert.match(appVersion,/function boot\(\)\{mount\(\);ensureEmployeesParityRunner\(\);/,'runner module must be requested during normal app boot');
...
assert.match(sw,/lat-yen-fresh-core-v3-authoritative-192/,'service worker release must invalidate cached false-positive parity runner bytes');
