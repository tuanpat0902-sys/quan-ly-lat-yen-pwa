...
assert.match(cache,/if\(options\.background&&interactionActive\(\)\)/,'background refresh must yield while the user is interacting');
...
assert.match(writes,/reason:'confirmed-write',allowDraftApply:true,render:false,forceApply:true,background:true/,'confirmed writes must refresh data without rebuilding the still-open form');
...
for(const marker of ['ly-vibe-read-cache.js?v=20260924.1','ly-vibe-business-writes.js?v=20260924.1','ly-performance-optimizer.js?v=20260924.1','ly-data-notifications.js?v=20260924.1'])assert.ok(loader.includes(marker),`missing deterministic runtime asset: ${marker}`);
