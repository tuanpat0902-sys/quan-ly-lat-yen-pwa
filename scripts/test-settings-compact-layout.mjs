...
assert.match(active,/<details class="settings-optional-section">[\s\S]*Thông báo trên thiết bị/,'notification preferences must be collapsed by default');
assert.match(active,/Sao lưu và bảo trì/,'backup and cleanup tools must be collapsed by default');
assert.doesNotMatch(active,/Kết nối lại Realtime|Smart Sync dự phòng/,'technical sync terminology and redundant reconnect controls must be removed');
...
assert.match(layout,/grid-template-areas:'cloud identity'/,'desktop settings must keep only the two primary cards visible');
assert.match(layout,/@media\(max-width:900px\)/,'settings layout must collapse responsively');
assert.match(enhancements,/ly-technical-settings/,'version information must be collapsed into one compact section');
assert.match(enhancements,/Thông tin phiên bản/,'the compact technical section must use user-facing wording');
assert.doesNotMatch(enhancements,/Production Gate|Consolidated Readiness|Chạy kiểm tra nhanh V3-2|shadow soak/,'migration diagnostics must not be rendered in user Settings');
assert.match(enhancements,/2026\.09\.21\.3/,'settings enhancement cache version must be current');
