...
assert.match(html, /function receiptRowsByKey[\s\S]*?sortReceiptRowsBySavedOrder/, 'receipt details must use the shared saved line order');
assert.match(html, /receipt\.rows=sortReceiptRowsBySavedOrder\(receipt\.rows\)/, 'history rendering must use the same saved line order as editing');
