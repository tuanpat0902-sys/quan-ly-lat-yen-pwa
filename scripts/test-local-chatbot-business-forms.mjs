...
const context={console,Date,Math,Promise,Intl,Event:class Event{},setTimeout(fn){fn();return 1;},document,window:{currentWarehouseId:'w1',db:{warehouses:[{id:'w1',name:'Kho chính'}],ingredients:[{id:'i1',warehouse_id:'w1',name:'Đường',unit:'kg',ingredient_type:'purchased'},{id:'i2',warehouse_id:'w1',name:'Sữa',unit:'lít',ingredient_type:'purchased'},{id:'i3',warehouse_id:'w1',name:'Bột cacao',unit:'g',purchase_unit:'hộp',conversion_ratio:500,ingredient_type:'purchased'},{id:'i4',warehouse_id:'w1',name:'Đá',unit:'kg',ingredient_type:'purchased'},{id:'i5',warehouse_id:'w1',name:'Nước đường',unit:'g',ingredient_type:'purchased'},{id:'i6',warehouse_id:'w1',name:'Syrup me',unit:'g',ingredient_type:'prepared'}],products:[{id:'p1',warehouse_id:'w1',name:'Cà phê sữa',unit:'ly'},{id:'p2',warehouse_id:'w1',name:'Cà phê đen',unit:'ly'},{id:'p3',warehouse_id:'w1',name:'Trà thanh xoài',unit:'ly'},{id:'p4',warehouse_id:'w1',name:'Yến nâu',unit:'ly'},{id:'p5',warehouse_id:'w1',name:'Yến đậm',unit:'ly'}],recipeItems:[{product_id:'p1',ingredient_id:'i1'},{product_id:'p2',ingredient_id:'i1'}]}},globalThis:null};
...
const distinctDiscountSale=assistant.assistantReply('Bán 15 cốc yến nâu giảm giá 1% và 15 cốc trà thanh xoài giảm giá 2% và 15 cốc yến đậm giảm giá 5%').draft;
await assistant.executeDraft(distinctDiscountSale);
const saleByProduct=id=>rows.sale.find(row=>row.querySelector('.srProduct').value===id);
assert.deepEqual(['p4','p3','p5'].map(id=>[saleByProduct(id).querySelector('.srQty').value,saleByProduct(id).querySelector('.srItemDiscountType').value,saleByProduct(id).querySelector('.srItemDiscountValue').value]),[['15','percent','1'],['15','percent','2'],['15','percent','5']],'the real sale form must receive the independent quantity and discount for every product');
assert.equal(elements.get('saleDiscountValue').value,'0','independent item discounts must leave the receipt discount empty');
