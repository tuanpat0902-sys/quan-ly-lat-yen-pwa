...
async function upsertSale(client,ctx,config,sale){
  ...
  await writeRow(client,'ly_sales',{id,org_id:ctx.org,warehouse_id:ctx.warehouse,receipt_no:receiptNo,sold_at:soldAt,source:'iPOS',note:sale.sale_note||'',subtotal,discount_type:'amount',discount_value:receiptDiscount,receipt_discount:receiptDiscount,item_discount_total:itemDiscount,discount,total_amount:total,ipos_tran_id:tranId,ipos_store_uid:config.storeUid,ipos_sale_updated_at:sale.sale_updated_at||null,ipos_payment_methods:sale.sale_payment_method||[],ipos_last_synced_at:new Date().toISOString()});
  await client.query(`delete from ${qi(schema)}.ly_sale_items where org_id=$1 and sale_id=$2`,[ctx.org,id]);
  for(const line of Array.isArray(sale.sale_detail)?sale.sale_detail:[]){const product=await upsertProduct(client,ctx,line);if(!product)continue;const quantity=number(line.quantity),unitPrice=number(line.price),lineSubtotal=number(line.price_org)*quantity,itemDiscountValue=number(line.discount_amount);await writeRow(client,'ly_sale_items',{org_id:ctx.org,sale_id:id,product_id:product.id,quantity,unit_price:unitPrice,line_subtotal:lineSubtotal,item_discount_type:'amount',item_discount_value:itemDiscountValue,item_discount:itemDiscountValue,line_total:number(line.amount),ipos_sale_detail_id:String(line.id_sale_detail||line.id||'')||null,ipos_item_id:String(line.item_id||'')||null,ipos_toppings:line.toppings||[]});}
  return {id,soldAt};
}

async function rebuildIposInventory(client,ctx){
  const old=await client.query(`select t.warehouse_id,t.ingredient_id,sum(t.quantity)::numeric as quantity from ${qi(schema)}.ly_stock_transactions t join ${qi(schema)}.ly_sales s on s.id=t.reference_id and s.org_id=t.org_id where t.org_id=$1 and s.source='iPOS' group by t.warehouse_id,t.ingredient_id`,[ctx.org]);
  await client.query(`delete from ${qi(schema)}.ly_stock_transactions t using ${qi(schema)}.ly_sales s where t.org_id=$1 and s.id=t.reference_id and s.org_id=t.org_id and s.source='iPOS'`,[ctx.org]);
