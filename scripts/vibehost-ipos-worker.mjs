...
function previousDay(label){const [year,month,day]=label.split('-').map(Number);return new Date(Date.UTC(year,month-1,day)-86_400_000).toISOString().slice(0,10);}
...
async function runSync({backfill=false,force=false}={}){
...
export function startVibeIposWorker(){if(process.env.VIBE_IPOS_SYNC!=='1'||timer)return;void runSync({backfill:true,force:true});timer=setInterval(()=>{if(activeHour())void runSync();},intervalMs);timer.unref?.();console.log('[ipos-vibe] enabled: Vibe PostgreSQL authoritative, 5-minute active schedule');}
export function requestVibeIposRecovery(){forcedRecovery=true;queueMicrotask(()=>void runSync({backfill:true,force:true}));}
