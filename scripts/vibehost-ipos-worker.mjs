...
export async function rebuildVibeIposInventory(client,ctx){
  ...
  const sales=await client.query(`select * from ${qi(schema)}.ly_sales where org_id=$1 and source='iPOS'`,[ctx.org]);
  const items=await client.query(`select i.* from ${qi(schema)}.ly_sale_items i join ${qi(schema)}.ly_sales s on s.id=i.sale_id where i.org_id=$1 and s.source='iPOS'`,[ctx.org]);
...
async function runSync({backfill=false,force=false,deep=false}={}){
  ...
  try{client=await getVibePool().connect();health=await syncHealth(client);if(!forced&&!mayAttempt(health))return;
    const completed=(await client.query(`select 1 from ${qi(schema)}.${qi('ly_runtime_sync_state')} where name='ipos_backfill' and value='complete'`)).rowCount>0,effectiveBackfill=backfill&&!completed,config=await iposConfig(client),ctx=await context(client),today=localDate(),lastDeep=(await client.query(`select value from ${qi(schema)}.${qi('ly_runtime_sync_state')} where name='ipos_deep_reconcile_day'`)).rows[0]?.value,deepRequested=deep&&(force||lastDeep!==today),lookbackDays=process.env.VIBE_IPOS_RECOVERY_LOOKBACK_DAYS||7,from=effectiveBackfill?(process.env.VIBE_IPOS_BACKFILL_FROM||'2026-08-25'):deepRequested?lookbackStartDay(today,lookbackDays):recoveryStartDay(health,today),days=labels(from),summary={catalog:0,sales:0,days:days.length,recovered:health.status==='degraded'||health.status==='needs_reconnect',deep:deepRequested};await client.query('begin');summary.catalog=await syncCatalog(client,ctx,config);for(const label of days){const window=dayWindow(label),headersForDay=(await saleHeaders(config,window)).filter(row=>row.deleted!==true&&String(row.store_uid||config.storeUid)===config.storeUid),details=[];for(let i=0;i<headersForDay.length;i+=detailConcurrency)details.push(...await Promise.all(headersForDay.slice(i,i+detailConcurrency).map(row=>saleDetail(config,row,window))));for(const sale of details){await upsertSale(client,ctx,config,sale);summary.sales++;}const active=[...new Set(headersForDay.map(row=>canonical(row.tran_id)))];const dayStart=new Date(window.start).toISOString(),dayEnd=new Date(window.end).toISOString();const stale=await client.query(`select id from ${qi(schema)}.ly_sales where org_id=$1 and source='iPOS' and sold_at between $2 and $3 and not(ipos_tran_id=any($4::text[]))`,[ctx.org,dayStart,dayEnd,active]);for(const row of stale.rows){await client.query(`delete from ${qi(schema)}.ly_sale_items where org_id=$1 and sale_id=$2`,[ctx.org,row.id]);await client.query(`delete from ${qi(schema)}.ly_sales where org_id=$1 and id=$2`,[ctx.org,row.id]);}}
    summary.activityEvents=await syncSaleActivityEvents(client,ctx);summary.inventoryGroups=await rebuildVibeIposInventory(client,ctx);if(effectiveBackfill)await client.query(`insert into ${qi(schema)}.${qi('ly_runtime_sync_state')}(name,value,updated_at) values('ipos_backfill','complete',now()) on conflict(name) do update set value='complete',updated_at=now()`);if(deepRequested)await client.query(`insert into ${qi(schema)}.${qi('ly_runtime_sync_state')}(name,value,updated_at) values('ipos_deep_reconcile_day',$1,now()) on conflict(name) do update set value=excluded.value,updated_at=now()`,[today]);await saveSyncHealth(client,successfulHealth(new Date(),summary,today));await client.query('commit');invalidateSnapshot();console.log(`[ipos-vibe] synchronized ${summary.sales} sale(s), ${summary.activityEvents} notification(s), ${summary.catalog} product(s), ${summary.days} day(s); recovered=${summary.recovered}; deep=${summary.deep}`);
