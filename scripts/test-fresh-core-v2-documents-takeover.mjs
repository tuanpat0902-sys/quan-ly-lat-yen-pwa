...
const previousCalls=[];const domainCalls=[];let loadCalls=0;let inventoryRefreshes=0;let indexInvalidations=0;let documentRenders=0;
...
context.db={inventory:[],movements:[]};context.invalidateDataIndexes=()=>{indexInvalidations++;};context.invalidateDerivedCaches=()=>{};context.renderImports=()=>{documentRenders++;};context.renderStocktake=()=>{documentRenders++;};context.loadCloud=async()=>{loadCalls++;return {legacy:true};};context.globalThis=context;
...
assert.ok(indexInvalidations>=3,'import, export and stocktake saves must invalidate cached receipt indexes');assert.ok(documentRenders>=3,'document menus must render immediately without a page reload');
