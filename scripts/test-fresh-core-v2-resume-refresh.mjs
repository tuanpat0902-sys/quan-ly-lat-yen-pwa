...
const source=await fs.readFile(new URL('../ly-fresh-core-v2-resume-refresh.js',import.meta.url),'utf8');
...
const context={
 ...
 window:{
   ...
   __lyFreshCoreV2:{async refreshCoreDomains(){order.push('refresh');refreshes++;await Promise.resolve();return true;}},
   __lyFreshCoreV2Shadow:{status(){return {phase,orgId};}},
   addEventListener(type,fn,capture){if(type==='visibilitychange'&&capture)visibleListener=fn;},
   removeEventListener(){},
 }
};
context.window.loadCloud=context.loadCloud;context.globalThis=context;
vm.createContext(context);vm.runInContext(source,context,{filename:'ly-fresh-core-v2-resume-refresh.js'});
const api=context.window.__lyFreshCoreV2ResumeRefresh;
assert.equal(api.status().enabled,true);
assert.equal(typeof visibleListener,'function');
visibleListener();
...
// Repeated foreground while the same refresh is pending should coalesce.
...
assert.equal(refreshes,2,'repeated foreground must share the same pending refresh');
assert.ok(api.status().coalesced>=1);

// Offline or non-ready V2 must leave the Legacy path untouched.
...
api.disable();assert.equal(api.status().enabled,false);await context.loadCloud();assert.equal(loads,6,'disable must restore prior loadCloud');
