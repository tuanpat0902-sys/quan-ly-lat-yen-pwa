...
export async function handleBusinessMutationApi(request,response,pathname){
  ...
  try{
    ...
    if(saleMatch&&request.method==='DELETE'){await client.query('begin');const result=await deleteSale(client,user,saleMatch[1]);if(!result.ok){await client.query('rollback');json(response,404,result);return true;}await client.query('commit');invalidateSnapshot(user.orgId);json(response,200,result);return true;}
    if(catalogMatch&&request.method==='DELETE'){await client.query('begin');const result=await deactivateCatalogItem(client,user,catalogMatch[1].toLowerCase(),catalogMatch[2]);if(!result.ok){await client.query('rollback');json(response,409,result);return true;}await client.query('commit');invalidateSnapshot(user.orgId);json(response,200,result);return true;}
    if(warehouseMatch&&request.method==='DELETE'){const input=await readBody(request).catch(()=>({}));await client.query('begin');const result=await deleteWarehouse(client,user,warehouseMatch[1],input.password);if(!result.ok){await client.query('rollback');json(response,409,result);return true;}await client.query('commit');invalidateSnapshot(user.orgId);json(response,200,result);return true;}
