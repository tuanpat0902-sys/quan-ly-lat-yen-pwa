...
export async function handleBusinessMutationApi(request,response,pathname){
  ...
  try{
    ...
    if(documentMatch&&request.method==='DELETE'){await client.query('begin');const kind=documentMatch[1].toLowerCase(),result=await deleteDocument(client,user,kind,documentMatch[2]);if(!result.ok){await client.query('rollback');json(response,404,result);return true;}await recordActivity(client,user,{table:{import:'ly_import_receipts',export:'ly_export_receipts',stocktake:'ly_stocktake_receipts'}[kind],id:result.id,type:'DELETE',name:result.receipt_no});await client.query('commit');invalidateSnapshot(user.orgId);json(response,200,result);return true;}
    if(saleMatch&&request.method==='DELETE'){await client.query('begin');const result=await deleteSale(client,user,saleMatch[1]);if(!result.ok){await client.query('rollback');json(response,404,result);return true;}await recordActivity(client,user,{table:'ly_sales',id:result.id,type:'DELETE',name:result.receipt_no});await client.query('commit');invalidateSnapshot(user.orgId);json(response,200,result);return true;}
    if(catalogMatch&&request.method==='DELETE'){await client.query('begin');const kind=catalogMatch[1].toLowerCase(),result=await deactivateCatalogItem(client,user,kind,catalogMatch[2]);if(!result.ok){await client.query('rollback');json(response,409,result);return true;}await recordActivity(client,user,{table:kind==='ingredient'?'ly_ingredients':'ly_products',id:result.id,type:'DELETE',name:result.row?.name});await client.query('commit');invalidateSnapshot(user.orgId);json(response,200,result);return true;}
