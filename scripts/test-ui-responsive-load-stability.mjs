...
assert.match(sales,/VERSION='2026\.08\.28\.3'/,'sales workflow scroll-fix version missing');
assert.match(sales,/data-ly-sales-revenue-card/,'revenue emphasis must remain');
assert.match(sales,/#saleReportArea \.scroll,#recentSalesArea \.scroll\{max-height:none!important;height:auto!important\}/,'sales report and recent-history scroll containers must size to content');
assert.match(sales,/layoutOwnership:'scroll-height-only'/,'sales UI must declare narrowly scoped scroll-height ownership');
...
assert.match(app,/UI_BUILD='UI-2026\.08\.28\.5'/,'visible sales scroll-fix marker missing');
...
assert.match(sw,/lat-yen-fresh-core-v3-authoritative-197/,'sales scroll fix must force a fresh service-worker release');
...
console.log('Responsive UI + accessibility + sales content-height scroll fix: PASS');
