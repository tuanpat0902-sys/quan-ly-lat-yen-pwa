...
const [pkg,sw,loader,readTakeover,hydration,manual,realtime,phase2,bootstrap,security,finalOwnership,cloud,domains,writeGuard]=await Promise.all([
 read('package.json'),read('sw.js'),read('ly-module-loader.js'),read('ly-fresh-core-v2-read-takeover.js'),read('ly-fresh-core-v2-legacy-hydration.js'),read('ly-fresh-core-v2-manual-refresh.js'),read('ly-fresh-core-v2-realtime.js'),read('ly-fresh-core-v2-realtime-phase2.js'),read('ly-independent-bootstrap.js'),read('ly-menu-security.js'),read('ly-fresh-core-v2-final-ownership.js'),read('ly-cloud-realtime.js'),read('src-v2/domains/create-domains.js'),read('scripts/legacy-direct-write-guard.mjs')
]);
...
assert.ok(realtime.includes('draftActive()')&&realtime.includes('deferredProjections'),'realtime open-receipt projection guard missing');
assert.ok(finalOwnership.includes('v240HasActiveDraft')&&finalOwnership.includes('scheduleProjection'),'final ownership open-receipt guard missing');
assert.ok(readTakeover.includes('deferredHydrations')&&manual.includes('deferredProjections'),'resume/manual open-receipt guards missing');
