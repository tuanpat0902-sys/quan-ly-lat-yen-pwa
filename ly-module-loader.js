(()=>{
  'use strict';
  if(window.__lyModuleLoaderV51)return;
  window.__lyModuleLoaderV51=true;
  ...
  const modules={
    ...
    appVersion:{src:'./ly-app-version.js?v=2.1.68',test:()=>window.__lyAppVersion?.version==='2.1.68'},
    localAssistant:{src:'./ly-local-chatbot.js?v=20260826.19',test:()=>window.__lyLocalAssistant?.version==='2026.08.26.19'},
    chatLanguagePlus:{src:'./ly-chat-language-plus.js?v=20260827.1',test:()=>window.__lyChatLanguagePlus?.version==='2026.08.27.1'},
  ...
  async function loadAssistant(){await load('localAssistant');await load('chatLanguagePlus');await load('chatLocalOnly');}
  ...
  function preparePanel(panel){
    ...
    if(panel==='history')load('activityHistory');if(panel==='employees'){load('employeesUI');load('employeeTerminationDate');}if(panel==='finance')load('financeUI');if(panel==='reports')load('reportsUI');if(panel==='cashflow')load('cashflowUI');if(HEAVY.has(panel))load('heavyPanels');
  }
