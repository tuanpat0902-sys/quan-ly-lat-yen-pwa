(()=>{
  'use strict';
  if(window.__lyModuleLoaderV10)return;
  window.__lyModuleLoaderV10=true;
  const VERSION='2026.08.24.3';
  const loaded=new Map();
  const HEAVY=new Set(['finance','employees','history','reports','settings','cashflow']);
  const modules={
    appVersion:{src:'./ly-app-version.js?v=2.0.4',test:()=>window.__lyAppVersion?.version==='2.0.4'},
  ...
  function load(name){
    ...
    const p=new Promise(resolve=>{const s=document.createElement('script');s.src=m.src;s.async=true;s.dataset.lyModule=name;s.onload=()=>resolve(true);s.onerror=()=>{loaded.delete(name);resolve(false)};(document.head||document.documentElement).appendChild(s);});loaded.set(name,p);return p;
  }
