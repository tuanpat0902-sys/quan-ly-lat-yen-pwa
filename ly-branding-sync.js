(()=>{
  ...
  const getOrgId=()=>{try{if(typeof v261OrganizationId!=='undefined'&&v261OrganizationId)return text(v261OrganizationId);}catch(e){}return text(window.__lyFreshOrgId||window.__lyOrganizationId||'');};

  function findBrandingCard(){
    const root=document.getElementById('settings')||document.querySelector('main')||document.body;
    return [...root.querySelectorAll('.card,section,article')].find(el=>/Nhận diện phần mềm/i.test(text(el.textContent))&&(/Tên phần mềm/i.test(text(el.textContent))||el.querySelector('input[type="text"]')))||null;
  }
  function cardParts(card=findBrandingCard()){
    if(!card)return {};
    const buttons=[...card.querySelectorAll('button')];
    return {card,nameInput:card.querySelector('input[type="text"],input:not([type])'),preview:card.querySelector('img'),fileInput:card.querySelector('input[type="file"]'),save:buttons.find(b=>/^Lưu$/i.test(text(b.textContent))),remove:buttons.find(b=>/^Xóa$/i.test(text(b.textContent)))};
  }
  ...
  async function imageSourceToData(src){src=text(src);if(!src||src==='about:blank')return null;if(src.startsWith('data:')&&src.length<=1450000)return src;try{const blob=await (await fetch(src,{cache:'no-store'})).blob();if(blob.size<=1050000)return await blobToData(blob);return await compressBlob(blob);}catch(e){return /^https?:/i.test(src)&&src.length<1400?src:null;}}
  ...
  function logoTargets(card){
    ...
    document.querySelectorAll('img[id*="logo" i],img[class*="logo" i],img[alt*="logo" i]').forEach(img=>{if(!card?.contains(img))targets.add(img);});
    return [...targets];
  }
  ...
  function applyFavicon(logo){
    if(!logo)return;
    let link=document.querySelector('link[rel="icon"][data-ly-branding]');
    if(!link){link=document.createElement('link');link.rel='icon';link.dataset.lyBranding='1';document.head.appendChild(link);}link.href=logo;
    let apple=document.querySelector('link[rel="apple-touch-icon"][data-ly-branding]');
    if(!apple){apple=document.createElement('link');apple.rel='apple-touch-icon';apple.dataset.lyBranding='1';document.head.appendChild(apple);}apple.href=logo;
  }
  function remountAppVersion(){try{window.__lyAppVersion?.mount?.();}catch(e){}}
  function applyBranding(row){
    if(!row)return;
    const name=text(row.software_name)||DEFAULT_NAME,logo=text(row.logo_data),parts=cardParts(),oldName=stripVersion(state.row?.software_name)||stripVersion(parts.nameInput?.value)||DEFAULT_NAME;
    state.applying=true;
    try{
      if(parts.nameInput&&parts.nameInput.value!==name)parts.nameInput.value=name;
      if(parts.preview){if(logo){if(parts.preview.getAttribute('src')!==logo)parts.preview.src=logo;parts.preview.style.display='';}else{parts.preview.removeAttribute('src');parts.preview.style.display='none';}}
      nameTargets(oldName).forEach(el=>{if(stripVersion(el.textContent)!==name)el.textContent=name;});
      logoTargets(parts.card).forEach(img=>{if(logo){if(img.getAttribute('src')!==logo)img.src=logo;img.style.display='';}else img.removeAttribute('src');});
