(()=>{
  ...
  function coreReady(){const core=window.__lyFreshCoreV2;return core?.domains?.imports?.save&&core?.domains?.exports?.save&&core?.domains?.stocktake?.save&&core?.domains?.inventory?.refresh&&core?.store?.getState?core:null;}

  function hydrateLegacy(){
    const core=coreReady();if(!core)return false;
    const s=core.store.getState()||{};
    const imports=s.importsData||{receipts:[],items:[]},exports=s.exportsData||{receipts:[],items:[]},stocktake=s.stocktakeData||{receipts:[],items:[]},inventory=s.inventoryData||{balances:[],transactions:[]};
    try{
  ...
  async function routedRpc(name,params,...rest){
    const domainName=SAVE_ROUTES[name];const deleteType=name==='ly_delete_receipt'?String(params?.p_type||'').toLowerCase():'';const deleteDomain=DELETE_ROUTES[deleteType];
    ...
    try{
      let data;
      if(domainName){const header=params?.p_header;const items=Array.isArray(params?.p_items)?params.p_items:[];data=await core.domains[domainName].save(header,items);}else data=await core.domains[deleteDomain].remove(params?.p_id);
    ...
    }catch(error){suppressNextLoadCloud=false;state.errors++;state.lastError=String(error?.message||error||'V2 document mutation failed');return {data:null,error,status:400,statusText:'V2 document mutation failed'};}
  }

  function enable(){if(state.enabled)return true;client=legacySupabase();if(!client||typeof client.rpc!=='function'||!coreReady()||!installLoadCloudGuard())return false;if(client.rpc?.__lyV2DocumentsTakeover)return true;previousRpc=client.rpc.bind(client);const wrapper=routedRpc;Object.defineProperty(wrapper,'__lyV2DocumentsTakeover',{value:true});client.rpc=wrapper;state.enabled=true;state.phase='active';state.lastError='';return true;}
  function disable(){if(client&&previousRpc&&client.rpc?.__lyV2DocumentsTakeover)client.rpc=previousRpc;try{if(originalLoadCloud&&typeof loadCloud==='function')loadCloud=originalLoadCloud;}catch(e){}suppressNextLoadCloud=false;state.enabled=false;state.phase='disabled';}
