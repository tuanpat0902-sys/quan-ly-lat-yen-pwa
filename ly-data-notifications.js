(()=>{
  ...
  const state={orgId:'',channel:null,pollTimer:null,startTimer:null,queue:[],flushTimer:null,cursor:0,ready:false,busy:false,sequence:0,clientId:'',userId:'',localTables:new Map()};
  ...
  function getClientId(){
    ...
    try{let id=localStorage.getItem(CLIENT_KEY);if(!id){id=makeId();localStorage.setItem(CLIENT_KEY,id);}state.clientId=id;return id;}
  ...
  function smartItem(row){
    ...
    const local=isLocalTable(table);
  ...
  async function reportTelemetry(status='',error=''){
    ...
    try{await client.from('ly_notification_devices').upsert(payload,{onConflict:'client_id'});}catch(e){}
  }

  async function showNative(item){
    if(!item.popup||!document.hidden)return false;
    if(!('Notification' in window)||Notification.permission!=='granted'){renderPermissionBanner();return false;}
    state.sequence++;
    ...
    try{const reg=await navigator.serviceWorker?.ready;if(reg?.showNotification){await reg.showNotification(item.title,options);await reportTelemetry('sw_show_success','');return true;}}
    catch(e){await reportTelemetry('sw_show_error',e?.message||String(e));}
    return false;
  }
  ...
  async function requestPermission(){
    ...
    try{const result=await Notification.requestPermission();await reportTelemetry(`permission_${result}`,'');if(result==='granted')removePermissionBanner();else renderPermissionBanner();}catch(e){await reportTelemetry('permission_request_error',e?.message||String(e));}
  }
  function renderPermissionBanner(){
    if(!document.body||!('Notification' in window)||Notification.permission==='granted'){removePermissionBanner();return;}
    let box=permissionBanner();if(!box){box=document.createElement('div');box.id='lyNotifyPermissionBannerV5';box.style.cssText='position:fixed;left:12px;right:12px;top:12px;z-index:2147483647;max-width:620px;margin:auto;background:#7c2d12;color:#fff;padding:12px 14px;border-radius:12px;box-shadow:0 18px 48px rgba(0,0,0,.28);font:13px/1.45 system-ui,-apple-system,Segoe UI,sans-serif;display:flex;gap:12px;align-items:center;justify-content:space-between';document.body.appendChild(box);}
    if(Notification.permission==='denied'){box.innerHTML='<span><b>Thông báo hệ thống đang bị chặn.</b> Trung tâm thông báo trong app vẫn hoạt động bình thường.</span>';return;}
    box.innerHTML='<span><b>Bật thông báo hệ thống?</b> Khi app chạy nền, các hoạt động quan trọng sẽ được báo.</span><button id="lyNotifyPermissionButtonV5" type="button" style="border:0;border-radius:9px;padding:8px 12px;font-weight:800;cursor:pointer;background:#fff;color:#7c2d12">Bật</button>';
    box.querySelector('#lyNotifyPermissionButtonV5')?.addEventListener('click',requestPermission,{once:true});
  }

  function dispatchItem(item,row){
    try{window.dispatchEvent(new CustomEvent('latyen:activity',{detail:{...item,row}}));}catch(e){}
    showNative(item);
  }
  ...
  function flush(){
    ...
    const rows=state.queue.splice(0).sort((a,b)=>(Number(a.id)||0)-(Number(b.id)||0));if(!rows.length)return;
    const primaryTables=new Set(rows.map(r=>text(r.entity_table)).filter(t=>PRIMARY.has(t)));
    const collapsed=new Map();
    for(const row of rows){
      const table=text(row?.entity_table),id=Number(row?.id)||0;if(!id||id<=state.cursor)continue;
      saveCursor(id);
      if(table==='ly_cashflow_entries'&&primaryTables.size)continue;
      const type=text(row?.event_type).toLowerCase(),entity=text(row?.entity_id)||String(id);
      const key=`${table}:${entity}:${type}`;
  ...
  async function establishBaseline(client){
    ...
    const {data,error}=await client.from('ly_activity_events').select('id').eq('org_id',state.orgId).order('id',{ascending:false}).limit(1);if(error)throw error;
  ...
  async function start(){
    ...
    if(state.ready&&state.orgId===org){renderPermissionBanner();return;}
    stopChannel();state.orgId=org;state.ready=false;getClientId();
    try{await establishBaseline(client);await reportTelemetry();}catch(e){clearTimeout(state.startTimer);state.startTimer=setTimeout(start,1200);return;}
    let ch=client.channel(`latyen-activity-v5-${org}-${Math.random().toString(36).slice(2,8)}`);
    ch=ch.on('postgres_changes',{event:'INSERT',schema:'public',table:'ly_activity_events',filter:`org_id=eq.${org}`},payload=>enqueue(payload?.new||{}));state.channel=ch;
    ch.subscribe(status=>{if(status==='SUBSCRIBED'){state.ready=true;reportTelemetry('realtime_subscribed','');poll();}if(status==='CHANNEL_ERROR'||status==='TIMED_OUT'||status==='CLOSED'){state.ready=false;if(state.channel===ch)state.channel=null;clearTimeout(state.startTimer);state.startTimer=setTimeout(start,1200);}});
    schedulePoll();renderPermissionBanner();
  }

  navigator.serviceWorker?.addEventListener('message',event=>{
    const data=event?.data||{};
    if(data.type==='LAT_YEN_LOCAL_MUTATION_COMMITTED'){markLocal(text(data.entityTable));return;}
    if(data.type==='LAT_YEN_NOTIFICATION_OPEN'&&data.panel){try{const btn=document.querySelector(`#nav button[data-panel="${CSS.escape(data.panel)}"]`);if(typeof window.showTab==='function')window.showTab(data.panel,btn||null);}catch(e){}}
  });
