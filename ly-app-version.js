...
function directBrandLabel(){
  ...
  const node=[...brand.childNodes].find(n=>n.nodeType===Node.TEXT_NODE&&names.has(stripVersion(n.textContent).toLowerCase()));
  const base=stripVersion(node?.textContent)||[...names][0]||DEFAULT_NAME;
  label=document.createElement('span');
  label.setAttribute('data-ly-brand-label','1');
  label.textContent=base;
  if(node){brand.insertBefore(label,node);node.remove();}
  else brand.insertBefore(label,brand.firstChild);
