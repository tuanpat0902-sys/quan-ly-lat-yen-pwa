...
(()=>{
  ...
  window.__lyCashflowModule={version:'2026.09.20.4'};
  ...
  function renderCashflow(forceReset=false){
    if(!E.cashflow)return;
    if(!forceReset&&cashflowFormOpen&&E.cashflow.querySelector('.cashflow-entry-card')){
      window.v240DeferOpenFormRender?.();
      return;
    }
  ...
  function renderCashflowReport(){
    ...
    const list=cashflowReportEntriesInRange()
