...
(()=>{
  ...
  window.__lyCashflowModule={version:'2026.09.19.1'};
  ...
  function renderCashflow(){
    ...
    if(typeof location!=='undefined'&&location.hostname.endsWith('.tinhgon.xyz'))window.__lyVibeBusinessWrites?.refreshCashflow?.().then(changed=>{if(changed&&activePanelId==='cashflow')renderCashflow();}).catch(error=>console.warn('[cashflow-history-read]',error));
  ...
  function projectFreshState(core=window.__lyFreshCoreV2){
    if(typeof location!=='undefined'&&location.hostname.endsWith('.tinhgon.xyz'))return false;
