(()=>{
  ...
  async function boot(){
    if(window.__lyFreshCoreV3Runtime?.core)return true;
    ...
    try{
      ...
      const [{createFreshCoreV3},{createRouter}]=await Promise.all([
        import('./src-v3/app/bootstrap.js?v=20260827.2'),
        import('./src-v3/app/router.js?v=20260827.1')
      ]);
      ...
      state.navigationAuthoritative=true;state.phase='ready';
  ...
  window.__lyFreshCoreV3Runtime={version:VERSION,boot,status:()=>({...state,core:!!window.__lyFreshCoreV3})};
  boot();
})();